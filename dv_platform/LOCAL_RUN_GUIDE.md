...
python3 -m dv_platform.automation --cases multi16
python3 -m dv_platform.automation --cases stream64
python3 -m dv_platform.automation --cases stream128
python3 -m dv_platform.automation --cases patterns
python3 -m dv_platform.automation --cases reset
python3 -m dv_platform.automation --cases reset_stream
```
...
退避间隔。平台会为烟雾和中等长度场景生成带回归 ID 的紧凑 VCD，随后可在“波形分析”页面
直接查看；64/128 字节深度流仅保留完整日志和 JSON 报告，避免时钟波形导致多 GB 文件。
“导入 VCD”按钮也会将本地 `.vcd` 文件上传并建立信号索引。
...
## 10. Cloudflare 免费公网演示链接

项目的推荐演示方式是 Cloudflare Quick Tunnel：免费、无需域名、无需开放路由器端口，访问者也
不需要与本机连接同一网络。每次重启会生成一个新的 `trycloudflare.com` 地址。
...
脚本会以 `Public URL (share this exact URL):` 明确打印当前可分享的
`https://*.trycloudflare.com` 地址。该地址仅在当前 Cloudflare Quick Tunnel 连接存活期间有效：
按 `Control + C`、网络断开或隧道自动重启后，必须使用脚本随后打印的新地址，旧地址不可复用。

该终端、前端终端和后端终端都必须持续运行。脚本启动前会检查页面和 `/api` 代理，因此打印出
分享地址即代表前端可访问且后端已通过代理连通。

请只分享当前打印的地址，并让一台不在本机网络中的设备打开它验证。旧地址、隧道重连前的地址，
以及脚本退出后的地址都会立即失效。若任何设备无法解析或连接 `trycloudflare.com`，请在该设备
切换未拦截 Cloudflare 的网络，或启用 Cloudflare WARP / 加密 DNS；这不是项目的前后端配置问题。
